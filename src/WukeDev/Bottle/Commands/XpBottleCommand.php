...
class XpBottleCommand extends Command implements PluginIdentifiableCommand{
    public $playerData = [];
    
    public function __construct()
    {
        parent::__construct("xpbottle", "Gives an exp Bottle which can be redeemed for exp levels", "/xpbottle <Levels> <Amount>(default 1)");
        $this->register($this->getPlugin()->getServer()->getCommandMap());
        $this->setPermission("use.xpbottle");
        $this->config = $this->getPlugin()->getConfig();
        $this->minimumLevel = $this->config->get("minimumLevel");
        $this->setAliases(["expb", "xpb", "bottle"]);
    }

    public function execute(CommandSender $sender, string $commandLabel, array $args)
    {
        
        if (!$sender->hasPermission("use.xpbottle"))
        {
        ...
        if ($args[0] == "all"){
            $levels = intval(floor(ExperienceUtils::getLevelFromXp($sender->getCurrentTotalXp())));
        ...
        try {
            ...
            //item object initialization
            $Bottle = new ExpBottle($levelInput);
